...
exports.deleteTestEvent = exports.fixScanRecords = exports.createEvent = exports.migrateScanRecords = exports.addErrorRecord = exports.addScanRecord = exports.getScanRecords = exports.getStudentById = exports.getStudents = exports.getEvents = void 0;
        ...
        if (scans.length === 0 && queryValue !== alternativeQueryValue) {
            ...
            try {
                const alternativeSnapshot = await db.collection("scans")
                    ...
                    .orderBy("timestamp", "desc")
                    .get();
                scans = alternativeSnapshot.docs.map((doc) => (Object.assign({ id: doc.id }, doc.data())));
...
exports.migrateScanRecords = functions.runWith({ invoker: 'public' }).https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
    res.set("Access-Control-Allow-Headers", "Content-Type");
    if (req.method === "OPTIONS") {
...
// Create Event
exports.createEvent = functions.runWith({ invoker: 'public' }).https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
    res.set("Access-Control-Allow-Headers", "Content-Type");
    if (req.method === "OPTIONS") {
    ...
    try {
        ...
        const createdEvent = Object.assign(Object.assign({ id: docRef.id }, eventDoc), { date: eventData.date || new Date().toISOString(), createdAt: new Date().toISOString() });
        res.status(201).json({
            success: true,
            event: createdEvent,
...
exports.fixScanRecords = functions.runWith({ invoker: 'public' }).https.onRequest(async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
    res.set("Access-Control-Allow-Headers", "Content-Type");
    if (req.method === "OPTIONS") {
